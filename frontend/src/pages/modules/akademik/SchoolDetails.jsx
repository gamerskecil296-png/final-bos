  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (res?.data?.top_sekolah) {
          setData(res.data.top_sekolah)
        }
      } catch (err) {
        console.error(err)
      } finally {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold font-display text-slate-800">Detail Asal Sekolah Mahasiswa</h1>
              <p className="text-sm text-slate-500 font-body">Menampilkan daftar seluruh sekolah asal pendaftar.</p>
            </div>
          </div>
          <div className="flex gap-2">
            <Badge className="bg-slate-100 text-slate-600 hover:bg-slate-200 text-sm py-1.5 px-3">
              Total Sekolah: {data.length}
            </Badge>
            <Badge className="bg-emerald-50 text-emerald-600 hover:bg-emerald-100 text-sm py-1.5 px-3 border-none">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col min-h-[500px]">
          ...
          <div className="flex-1 overflow-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-slate-400">
                <p>Tidak ada sekolah yang sesuai pencarian.</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 sticky top-0 shadow-sm z-10">
                  <tr>
                    ...
                    <th className="py-3 px-6 text-xs font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100">Nama Sekolah</th>
                  ...
                  {filteredData.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-3 px-6 text-sm text-emerald-600 font-bold text-right">{item.count}</td>
