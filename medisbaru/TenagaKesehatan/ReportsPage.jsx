  ...
  const handleExport = async ({ startDate, endDate }) => {
    ...
    try {
      ...
      if (exportModalConfig.type === 'excel') {
        response = await healthReportsService.exportExcel({ start_date: startDate, end_date: endDate });
        filename = `Laporan_Analitik_${startDate}_sd_${endDate}.xlsx`;
      } else if (exportModalConfig.type === 'pdf') {
        response = await healthReportsService.exportPDF({ start_date: startDate, end_date: endDate });
